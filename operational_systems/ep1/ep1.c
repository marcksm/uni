...
void fcfsScheduler (FILE *output) {
    ...
    while (i < quantProcess) {
        ...
        if (execute) {
            ...
            fprintf (output, "%s %f %f\n", exePr.name, (double) tf / 1000000,
                    (double) (tf - realt0) / 1000000);
            execute = 0;
        }
    }
}
    ...
    while (isExe || !isEmpty (pQueue) || j < quantProcess) {
        ...
        if (j < quantProcess) {
                ...
                if (processFinished) {
                    ...
                    if (extraInfo)
                        fprintf (stderr, "Instante %fs: Processo \"%s\""
                                 ...
                                 exePr.name, i);
                    fprintf (output, "%s %f %f\n", exePr.name,
                            (double) t / 1000000, (double) (t - exePr.t0) /
                            1000000);
                    i++;
                }
                pthread_mutex_unlock (&processFinishedMutex);
            ...
            if (isExe) {
                ...
                fprintf (output, "%s %f %f\n", exePr.name, (double) t / 1000000,
                        (double) (t - exePr.t0) / 1000000);
                i++;
            }
            if (!isEmpty (pQueue)) {
                exePr = dequeue (pQueue);
...
int main (int argc, char **argv) {
    ...
    extraInfo = 0;
    if (argc < 4)
        fatalError ("Incorrect usage. Use: ep1 schedulerType traceFileName"
                    " outputFileName [d]");
    ...
    if (sscanf (argv[1], "%d", &schedulerType) == EOF)
        fatalError ("Could't get scheduler type");
