...
public class KosarajuSharirSCC {
    ...
    public static void main(String[] args) {
        ...
        TransitiveClosure tcc = new TransitiveClosure(G);
        boolean quit = false;
      ...
      for (int v : components[M-1]) {
          for (int x: components[0]) {
              if (tcc.reachable(v, x)) { 
                  quit = true; 
                  for (int k : components[M-1])  StdOut.print(k + " ");
              }
          }
          if (quit)  StdOut.println(); break;
      }
      if (!quit) StdOut.println ("DO NOT HAVE CORE");
    }
